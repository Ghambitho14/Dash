...
import { getStorageObject, setStorageObject, removeStorageItem } from './utils/storage';
import toast from 'react-hot-toast';
import { logger } from './utils/logger';

// Exponer configuración de Supabase para el servicio de burbuja flotante
if (typeof window !== 'undefined' && window.Capacitor && window.Capacitor.isNativePlatform()) {
	const supabaseUrl = import.meta.env.VITE_PROJECT_URL;
	const supabaseKey = import.meta.env.VITE_ANNON_KEY;
	
	if (supabaseUrl && supabaseKey) {
		// Guardar en localStorage para que el WebView del servicio pueda acceder
		try {
			localStorage.setItem('supabase_url', supabaseUrl);
			localStorage.setItem('supabase_key', supabaseKey);
		} catch (e) {
			console.warn('No se pudo guardar configuración de Supabase en localStorage:', e);
		}
	}
}
