...
import { getStatusColor, getStatusIcon, useCurrentTime, formatRelativeTime, formatPrice } from '../../utils/utils';
...
export function OrderCard({ order, onClick, onDelete, onAcceptOrder, onUpdateStatus }) {
	...
	const handleVoyEnCamino = (e) => {
		...
		const check = validateOrderForTransition(order, 'En camino');
		if (!check.ok) {
			toast.error(check.reason);
			return;
		}

		onUpdateStatus(order.id, 'En camino');
			...
			<div className="order-card-details">
				...
				<div className="order-card-location-wrapper">
					<div className="order-card-detail">
						...
						<span className="order-card-detail-text">{order.deliveryAddress}</span>
					</div>
					<motion.button
			...
			{order.deliveryAddress && (
				<div className="order-card-map-container">
					<iframe
						...
						loading="lazy"
						src={`https://www.google.com/maps/embed/v1/place?key=AIzaSyBFw0Qbyq9zTFTd-tUY6dZWTgaQzuU17R8&q=${encodeURIComponent(order.deliveryAddress)}&zoom=15`}
						title={`Mapa de ${order.deliveryAddress}`}
						allowFullScreen
					/>
				</div>
			)}
