...
export function useLocationTracking(driverId, orderId = null, enabled = true) {
	...
	useEffect(() => {
		...
		const initTracking = async () => {
			...
			const options = {
				enableHighAccuracy: true,
				timeout: 20000, // Aumentado a 20 segundos para navegador
				maximumAge: 0,
			};
			
			// Función para obtener ubicación inicial con reintentos
			const getInitialPosition = (attempt = 1, maxAttempts = 3) => {
				logger.log(`📍 Solicitando ubicación inicial (navegador) - Intento ${attempt}/${maxAttempts}...`);
				...
				navigator.geolocation.getCurrentPosition(
					...
					(error) => {
						...
						if (error.code === 3 && attempt < maxAttempts) {
							...
							setTimeout(() => {
								getInitialPosition(attempt + 1, maxAttempts);
			...
			const watchOptions = {
				enableHighAccuracy: true,
				timeout: 30000, // 30 segundos para watchPosition (más permisivo)
